> **🌟 美团 CatPaw 定制版本**
>
> OpenSpec 在美团 CatPaw IDE 中的定制版本。支持 11 个完整工作流命令（`/opsx:*` 格式）直接在 CatPaw 中使用。
...
- **模型推荐**：使用高推理能力的模型（如 Claude Opus）效果最佳
- **上下文管理**：保持清洁的上下文窗口，实施前清空历史记录
...
- 构建：`pnpm run build`
- 测试：`pnpm test`
- 本地开发：`pnpm run dev` 或 `pnpm run dev:cli`
